...
namespace LeaseIt.Data
{
    public static class DbInitializer
    {
        ...
        {            
            ...
            {
                var result = await roleManager.CreateAsync(new IdentityRole(UserRoles.User));
                if (!result.Succeeded)
                    return 2;  // should log an error message here            
            }
            return 0;
        ...
        {   // Create Admin User
            ...
            result = await userManager.AddToRoleAsync(adminUser, UserRoles.Admin);
            if (!result.Succeeded)
                return 2;  // should log an error message here           
            ...
            result = await userManager.AddToRoleAsync(memberUser, UserRoles.User);
            if (!result.Succeeded)
                return 4;  // should log an error message here
